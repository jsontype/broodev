# PEPE_SIGNAL // buy-timing terminal ₿

실시간 페페 지표 6종을 합성해 **"지금이 살 타이밍인가?"**를 0~100 점수로 보여주는 해커 감성 대시보드 (React · 빌드 불필요).
...
- PEPE 현재가·24h·거래량·시총 + 가격 스파크라인 + 점수 추이
...
- 합성 점수/가중치는 일반적 해석에 기반한 **출발점**이며, 실제 운용 전 페페 일봉 백테스트로 보정 필요.
